...
func TestAdminLookupByLabelUsesExactFailClosedIndex(t *testing.T) {
	previousDB := db
	previousIndex := adminPasswordByLabel
	t.Cleanup(func() {
		db = previousDB
		adminPasswordByLabel = previousIndex
	})
	label := "lr1-abcdefghijklmnopqrstuvwxyz234567"
	db = &Database{Passwords: map[string]*PasswordEntry{
		"ClientSecret1234": {Label: label},
	}}
	rebuildAdminLabelIndexLocked()
	response, err := adminPasswordByExactLabel(db, []string{"--label", label})
	if err != nil || response.Password == nil || response.Password.Password != "ClientSecret1234" {
		t.Fatalf("exact lookup failed: response=%#v err=%v", response, err)
	}
	db.Passwords["AnotherSecret123"] = &PasswordEntry{Label: label}
	rebuildAdminLabelIndexLocked()
	if _, err := adminPasswordByExactLabel(db, []string{"--label", label}); err == nil {
		t.Fatal("duplicate label did not fail closed")
	}
}

func TestSaveAdminDBPublishesLiveLabelIndexOnlyAfterPersistence(t *testing.T) {
	previousDB := db
	previousIndex := adminPasswordByLabel
	t.Cleanup(func() {
		db = previousDB
	...
	firstLabel := "lr1-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	secondLabel := "lr1-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
	db = &Database{
		MainPassword: "owner-secret-for-test",
		Passwords: map[string]*PasswordEntry{
			"ClientSecret1234": {Label: firstLabel},
		},
		Devices: make(map[string]*ClientDevice),
	}
	adminPasswordByLabel = map[string]string{"unchanged": "sentinel"}

	directory := canonicalTempDir(t)
	if err := saveAdminDB(directory, db); err != nil {
		t.Fatal(err)
	}
	if got := adminPasswordByLabel[firstLabel]; got != "ClientSecret1234" {
		t.Fatalf("persisted label index=%q, want ClientSecret1234", got)
	}
	if _, exists := adminPasswordByLabel["unchanged"]; exists {
		t.Fatal("stale label survived successful persistence")
	}

	db.Passwords["ClientSecret1234"].Label = secondLabel
	adminPasswordByLabel = map[string]string{firstLabel: "ClientSecret1234"}
	blockingFile := filepath.Join(directory, "not-a-directory")
	if err := os.WriteFile(blockingFile, []byte("block"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := saveAdminDB(blockingFile, db); err == nil {
		t.Fatal("save through a non-directory unexpectedly succeeded")
	}
	if got := adminPasswordByLabel[firstLabel]; got != "ClientSecret1234" {
		t.Fatal("failed persistence changed the live label index")
	}
	if _, exists := adminPasswordByLabel[secondLabel]; exists {
		t.Fatal("failed persistence published an uncommitted label")
	}
}

func TestExplicitAdminSocketRejectsRelativePath(t *testing.T) {
	if _, _, err := explicitAdminSocketPath(canonicalTempDir(t), "admin.sock"); err == nil {
		t.Fatal("relative explicit admin socket accepted")
	}
}

func TestExplicitAdminSocketRequiresRootOwnedPrivateParent(t *testing.T) {
	if os.Geteuid() != 0 {
		t.Skip("root ownership assertions require a root test container")
	}
	directory, err := os.MkdirTemp("/tmp", "levik-admin-socket-")
	if err != nil {
		t.Fatal(err)
	}
	...
	selected, explicit, err := explicitAdminSocketPath("/unused", path)
	if err != nil || !explicit || selected != path {
		t.Fatalf("safe explicit admin path rejected: selected=%q explicit=%v err=%v", selected, explicit, err)
	}
	if err := os.Chmod(directory, 0750); err != nil {
		t.Fatal(err)
	}
	if _, _, err := explicitAdminSocketPath("/unused", path); err == nil {
		t.Fatal("group-accessible admin parent accepted")
	}
}
...
func TestAdminServerInfoReportsRunningBinarySHA256(t *testing.T) {
	...
	info := buildAdminServerInfo(canonicalTempDir(t), &Database{
...
func TestAdminServerInfoSeparatesClientAndOwnerDevices(t *testing.T) {
	...
	info := buildAdminServerInfo(canonicalTempDir(t), loaded)
...
func TestAdminCreateRetentionSurvivesRenewal(t *testing.T) {
	configDir := canonicalTempDir(t)
	loaded := &Database{
		MainPassword: "ABCDEFGHJKLMNPQR",
		DefaultPorts: "56000,56001,9000",
	...
	password := "23456789ABCDEFGH"
	expiresAt := time.Now().Add(time.Hour).Unix()
	const retentionSeconds = int64(48 * time.Hour / time.Second)
	purgeAfter := expiresAt + retentionSeconds
	response, err := adminCreatePassword(configDir, loaded, []string{
		"--client-password", password,
		"--expires-at", strconv.FormatInt(expiresAt, 10),
		"--purge-after", strconv.FormatInt(purgeAfter, 10),
	})
	if err != nil || response.Password == nil {
		t.Fatalf("retained password create failed: response=%#v err=%v", response, err)
	}
	entry := loaded.Passwords[password]
	if entry == nil || entry.PurgeAfter != purgeAfter || isPasswordPurgeableAt(entry, expiresAt) || !isPasswordPurgeableAt(entry, purgeAfter) {
		t.Fatalf("retention deadline was not enforced: %#v", entry)
	}

	renewedExpiresAt := expiresAt + int64(12*time.Hour/time.Second)
	setPasswordExpiryPreservingRetention(entry, renewedExpiresAt)
	if entry.ExpiresAt != renewedExpiresAt || entry.PurgeAfter != renewedExpiresAt+retentionSeconds {
		t.Fatalf("renewal did not preserve retention window: %#v", entry)
	}
}

func TestPasswordRetentionDeadlineAndRenewal(t *testing.T) {
	const retentionSeconds = int64(48 * time.Hour / time.Second)
	entry := &PasswordEntry{ExpiresAt: 1_900_000_000, PurgeAfter: 1_900_000_000 + retentionSeconds}
	if isPasswordPurgeableAt(entry, entry.ExpiresAt) {
		t.Fatal("entry was purged at expiry despite retention grace")
	}
	if !isPasswordPurgeableAt(entry, entry.PurgeAfter) {
		t.Fatal("entry was not purgeable at retention deadline")
	}
	newExpiry := entry.ExpiresAt + 3600
	setPasswordExpiryPreservingRetention(entry, newExpiry)
	if entry.ExpiresAt != newExpiry || entry.PurgeAfter != newExpiry+retentionSeconds {
		t.Fatalf("retention was not preserved across renewal: %#v", entry)
	}
}
