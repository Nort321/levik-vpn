...
func runAdminCLI(args []string) int {
	...
	mainPasswordFile := fs.String("main-password-file", "", "absolute owner-only file with the admin password")
	adminSocket := fs.String("admin-socket", "", "explicit absolute local admin socket path")
	offline := fs.Bool("offline", false, "изменить остановленный сервер напрямую (потребуется запуск сервиса)")
	if err := fs.Parse(args); err != nil {
		writeAdminError(err)
		return 2
	}
	mainPasswordValue, err := chooseFileSecret(*mainPassword, *mainPasswordFile, "main-password", true)
	if err != nil {
		writeAdminError(err)
		return 2
	}
	...
	if !*offline {
		response, err := callAdminSocketAt(*configDir, *adminSocket, adminRequest{
	...
	if loaded.MainPassword != mainPasswordValue {
...
func executeAdminCommand(configDir string, loaded *Database, rest []string, wgDev wgDevice, live bool) (adminResponse, error) {
	...
	case "lookup-label":
		response, err = adminPasswordByExactLabel(loaded, rest[1:])
...
func adminPasswordByExactLabel(loaded *Database, args []string) (adminResponse, error) {
	fs := flag.NewFlagSet("lookup-label", flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	label := fs.String("label", "", "exact client label")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 {
		return adminResponse{}, newAdminCommandError("invalid", "invalid lookup-label request")
	}
	value := strings.TrimSpace(*label)
	if value == "" || value != normalizePasswordLabel(value) {
		return adminResponse{}, newAdminCommandError("invalid", "invalid exact label")
	}

	password := ""
	found := false
	if loaded == db {
		password, found = adminPasswordByLabel[value]
	} else {
		// Offline CLI is not on the node-agent hot path.
		for candidate, entry := range loaded.Passwords {
			if entry == nil || strings.TrimSpace(entry.Label) != value {
				continue
			}
			if found {
				return adminResponse{}, newAdminCommandError("label_conflict", "duplicate label")
			}
			password, found = candidate, true
		}
	}
	if !found {
		return adminResponse{}, newAdminCommandError("not_found", "label not found")
	}
	if password == "" {
		return adminResponse{}, newAdminCommandError("label_conflict", "duplicate label")
	}
	...
	if entry == nil {
		return adminResponse{}, newAdminCommandError("state_conflict", "indexed label is unavailable")
	}
...
func explicitAdminSocketPath(configDir, override string) (string, bool, error) {
	override = strings.TrimSpace(override)
	if override == "" {
		return adminSocketPath(configDir), false, nil
	}
	if !filepath.IsAbs(override) || filepath.Clean(override) != override || filepath.Base(override) == "." {
		return "", true, errors.New("admin socket path must be canonical and absolute")
	}
	parent := filepath.Dir(override)
	canonical, err := filepath.EvalSymlinks(parent)
	if err != nil || canonical != parent {
		return "", true, errors.New("admin socket parent is not canonical")
	}
	info, err := os.Lstat(parent)
	if err != nil || !info.IsDir() || info.Mode().Perm() != 0700 || !rootOwnedSecretPath(info, false) {
		return "", true, errors.New("admin socket parent must be root-owned mode 0700")
	}
	return override, true, nil
}

func validateExplicitAdminSocketFile(path string) error {
	info, err := os.Lstat(path)
	if err != nil {
		return err
	}
	if info.Mode()&os.ModeSocket == 0 || info.Mode().Perm() != 0600 || !rootOwnedSecretPath(info, true) {
		return errors.New("admin socket must be root-owned mode 0600")
	}
	return nil
}

func callAdminSocket(configDir string, request adminRequest) (adminResponse, error) {
	return callAdminSocketAt(configDir, "", request)
}

func callAdminSocketAt(configDir, override string, request adminRequest) (adminResponse, error) {
	path, explicit, err := explicitAdminSocketPath(configDir, override)
	if err != nil {
		return adminResponse{}, err
	}
	if explicit {
		if err := validateExplicitAdminSocketFile(path); err != nil {
			return adminResponse{}, err
		}
	}
...
func startAdminSocket(ctx context.Context, configDir string, wgDev wgDevice) error {
	return startAdminSocketAt(ctx, configDir, "", wgDev)
}

func startAdminSocketAt(ctx context.Context, configDir, override string, wgDev wgDevice) error {
	path, explicit, err := explicitAdminSocketPath(configDir, override)
	if err != nil {
		return err
	}
	...
	if existing, statErr := os.Lstat(path); statErr == nil {
		if explicit && (existing.Mode()&os.ModeSocket == 0 || existing.Mode().Perm() != 0600 || !rootOwnedSecretPath(existing, true)) {
			return errors.New("refusing to replace unsafe admin socket path")
		}
...
func readAdminDB(configDir string) (*Database, error) {
	...
	data, err := readRootPrivateFile(path, maxSecureDatabaseBytes)
	if err != nil {
	...
	if err := validateMaxGeneratedPasswords(loaded.MaxPasswords); err != nil {
		return nil, fmt.Errorf("invalid_max_passwords: %w", err)
	}
	return &loaded, nil
}
...
func saveAdminDB(configDir string, loaded *Database) error {
	...
	path := filepath.Join(configDir, "passwords.json")
	if err := writeRootPrivateFileAtomic(path, data); err != nil {
		return err
	}
	// Live admin mutations run while dbMutex is held. Publish their exact-label
	// lookup index only after durable persistence succeeds, so node-agent status
	// observes a newly created/renamed credential immediately and a failed write
	// can never expose an index for state that was not committed.
	if loaded == db {
...
func adminSetLimit(configDir string, loaded *Database, args []string) (adminResponse, error) {
	...
	if err := validateMaxGeneratedPasswords(*value); err != nil {
		return adminResponse{}, err
	}
...
func adminUpdateSettings(configDir string, loaded *Database, args []string) (adminResponse, error) {
	...
	if err := validateMaxGeneratedPasswords(*limit); err != nil {
		return adminResponse{}, err
	}
	normalizedPorts, err := parsePortsSpec(*ports)
	if err != nil {
