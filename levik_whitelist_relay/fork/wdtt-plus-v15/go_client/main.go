...
func main() {
	...
	vkAuthMode := flag.String("vk-auth", "anonymous", "режим VK авторизации (anonymous/account)")
	levikControlSocket := flag.String("levik-control-sock", "", "Levik abstract Unix control socket; all sensitive options arrive through it")
	...
	if strings.TrimSpace(*levikControlSocket) != "" {
		if *connPassword != "" || *vkHash != "" || *rtMasque || os.Getenv("WDTT_CUSTOM_VK_CLIENT_ID") != "" || os.Getenv("WDTT_CUSTOM_VK_CLIENT_SECRET") != "" {
			log.Fatal("[LEVIK] Секреты/профиль в argv или env запрещены в Levik mode")
		}
		control, init, err := acceptLevikControl(ctx, strings.TrimSpace(*levikControlSocket))
		if err != nil {
			log.Fatalf("[LEVIK] Control IPC: %v", err)
		}
		levikControlClient = control
		defer levikControlClient.close()
		if err := silenceLevikProcessLogs(); err != nil {
			levikControlClient.fail("log_sink_failed", nil)
			os.Exit(1)
		}
		*peerAddr = init.Peer
		*listen = "127.0.0.1:0"
		*host = init.TurnHost
		*port = init.TurnPort
		*vkHash = strings.Join(init.VKHashes, ",")
		*connPassword = init.Password
		*deviceID = init.DeviceID
		*deviceInfo = string(init.DeviceInfo)
		*transportSessionFlag = init.TransportSession
		if init.Workers != 0 {
			*numW = init.Workers
		}
		if init.CaptchaMode != "" {
			*captchaMode = init.CaptchaMode
		}
		*turnStreamFirst = init.TurnStreamFirst
		*turnSNI = init.TurnSNI
		if init.Fingerprint != "" {
		...
		*vkAuthMode = init.VKAuthMode
		...
		if err != nil {
			levikControlClient.fail("protect_channel_failed", err)
			log.Fatalf("[LEVIK] Protect IPC: %v", err)
		}
		defer levikProtector.close()
	}
	if err := setupGlobalResolver(); err != nil && levikControlClient != nil {
		levikControlClient.fail("dns_route_unavailable", err)
		log.Fatalf("[LEVIK] DNS: %v", err)
	}
	if levikControlClient != nil {
		go levikControlClient.runCommands(ctx, func(command levikControlCommand) {
			switch command.Type {
			case "STOP":
				cancel()
			case "PAUSE":
			...
			case "CAPTCHA_RESULT":
				enqueueCaptchaResult(CaptchaResult{RequestID: strings.TrimSpace(command.RequestID), Value: strings.TrimSpace(command.Value)})
			case "TURN_CREDS":
				if err := deliverAccountTurnCredentials(command); err != nil {
					levikControlClient.fail("invalid_turn_credentials", nil)
				}
			default:
				levikControlClient.fail("unknown_command", nil)
			}
		})
	}
	...
	if levikControlClient != nil {
		customClientID = levikCustomClientID
		customClientSecret = levikCustomClientSecret
	}
	if *clientIdsFlag != "" {
	...
	var wrapV2ID *[levikWrapIDLen]byte
	if levikControlClient != nil {
		id := deriveLevikWrapID(*connPassword)
		wrapV2ID = &id
	}
	...
	if levikControlClient != nil {
		levikControlClient.ready("TRANSPORT_LISTENING", map[string]any{"localPort": localPort})
	}
	...
	stats := NewStats()
	if levikControlClient != nil {
		go levikControlClient.runStats(ctx, stats)
	}
		...
		case rawConf, ok := <-configCh:
			...
			if levikControlClient != nil {
				if err := startLevikWireGuard(
					...
					levikControlClient.fail(code, err)
					cancel()
				}
				return
			}
	...
	for g := 0; g < numGroups; g++ {
		...
		if remaining := *numW - g*workersPerGroup; remaining < workerCount {
			workerCount = remaining
		}
		ids := make([]int, workerCount)
		for i := range ids {
...
func silenceLevikProcessLogs() error {
