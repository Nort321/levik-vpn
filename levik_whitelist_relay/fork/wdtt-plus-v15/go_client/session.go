...
const (
	...
	turnAllocationTimeout        = 12 * time.Second
	// Pion's default 200 ms RTO takes about 25 seconds to exhaust seven
	// exponential-backoff retransmissions. That can consume Android's entire
	// preparation budget before fallback TURN transports are attempted.
	turnAllocationRTO = 75 * time.Millisecond
)
...
func verifyTURNCertificateForEndpoint(
	...
	if endpointHost == "" {
		return fmt.Errorf("пустое имя TURN ендпоинта для проверки TLS")
	}
	...
	if err != nil {
		return fmt.Errorf("проверка цепочки и имени сертификата TURN TLS для %q: %w", endpointHost, err)
	}
	return nil
}
...
func turnTLSConfig(endpoint turnEndpoint, frontSNI string) *tls.Config {
	...
	// crypto/tls does not put IP literals into ClientHello SNI, but it still
	// needs ServerName to verify an IP SAN when the caller uses tls.Client.
	config.ServerName = serverName

	// При подмене SNI сертификат всё равно должен аутентифицировать реальный
	// TURN-узел. Отключаем стандартную проверку имени только потому, что ServerName содержит
	// внешний SNI; VerifyConnection ниже обязательно проверяет и CA-цепочку, и endpoint.Host/IP.
	if frontSNI != "" && !strings.EqualFold(frontSNI, endpoint.Host) {
		...
		config.VerifyConnection = func(state tls.ConnectionState) error {
			return verifyTURNCertificateForEndpoint(state, endpoint.Host, config.RootCAs)
		}
	}
	return config
}
...
func openTURNAllocation(
	...
	case turnTransportUDP:
		dialer := protectedDialer(6*time.Second, 30*time.Second)
		rawConn, err := dialer.DialContext(ctx, "udp", turnAddr)
		if err != nil {
			return nil, nil, fmt.Errorf("TURN UDP подключение %s: %w", turnAddr, err)
		}
		c, ok := rawConn.(*net.UDPConn)
		if !ok {
			rawConn.Close()
			return nil, nil, errors.New("TURN UDP dial returned non-UDP socket")
		}
	...
	case turnTransportTLS:
		...
		defer cancel()
		protected := protectedDialer(8*time.Second, 30*time.Second)
		dialer := &tls.Dialer{
			NetDialer: &protected,
	...
	return allocateTURNOnConn(ctx, endpoint, peer, creds, turnConn)
}
...
func allocateTURNOnConn(
	...
	turnAddr := endpoint.address()
	if err := turnConn.SetDeadline(turnAllocationDeadline(ctx, time.Now())); err != nil {
		_ = turnConn.Close()
		return nil, nil, fmt.Errorf("TURN %s deadline %s: %w", endpoint.label(), turnAddr, err)
	}
	...
	if err := turnConn.SetDeadline(time.Time{}); err != nil {
		tc.Close()
		return nil, nil, fmt.Errorf("TURN %s clear deadline %s: %w", endpoint.label(), turnAddr, err)
	}

	return tc, relay, nil
}

func turnAllocationDeadline(ctx context.Context, now time.Time) time.Time {
	deadline := now.Add(turnAllocationTimeout)
...
func openTURNAllocationOverMasque(
	...
	return allocateTURNOnConn(ctx, endpoint, peer, creds, turn.NewSTUNConn(conn))
}
...
func RunSession(
	...
	candidates = rotateTURNCandidates(candidates, candidateOffset)
	if len(candidates) == 0 {
	...
	for idx, candidate := range directCandidates {
		emitTURNDiagnostic(candidate.Transport, "attempt")
		...
		lastTURNErr = err
		emitTURNDiagnostic(candidate.Transport, "failed")
	...
	accountControl.Load().diagnostic("turn_allocation_ready")
	defer tc.Close()
	defer relay.Close()
	...
	var replayWindow levikReplayWindow
	if useWrap {
	...
	go func() {
		...
		for {
			...
			if useWrap {
				...
				var wrapErr error
				if tp.WrapV2ID != nil {
					m, wrapErr = obfsUnwrapPacketV2(tp.WrapKey, *tp.WrapV2ID, payload, plain)
					if wrapErr == nil && !replayWindow.Accept(payload) {
						wrapErr = errors.New("WRAP v2 replay rejected")
					}
				} else {
		...
		for {
			...
			if useWrap {
				if obfsCfg != nil && obfsWriteState != nil {
					...
					var wrapErr error
					if tp.WrapV2ID != nil {
						wrapped, wrapErr = obfsWrapPacketV2(tp.WrapKey, *tp.WrapV2ID, out, obfsCfg, obfsWriteState)
					} else {
	...
	accountControl.Load().diagnostic("dtls_handshake_started")
	...
	if err != nil {
		accountControl.Load().diagnostic("dtls_handshake_failed")
		if useWrap {
	...
	accountControl.Load().diagnostic("dtls_handshake_ready")
	...
	if getConfig && configCh != nil {
		...
		if confErr != nil {
			...
			accountControl.Load().diagnostic("relay_config_received")
			select {
			case configCh <- conf:
...
func rotateTURNCandidates(candidates []turnEndpoint, offset int) []turnEndpoint {
	if len(candidates) < 2 {
		return candidates
	}
	...
	rotated := make([]turnEndpoint, 0, len(candidates))
	rotated = append(rotated, candidates[offset:]...)
	rotated = append(rotated, candidates[:offset]...)
	return rotated
}

func emitTURNDiagnostic(transport turnTransport, stage string) {
	control := accountControl.Load()
	if control == nil {
		return
	}
	code := "turn_" + string(transport) + "_" + stage
