...
func TestRotateTURNCandidatesAdvancesAfterEndToEndFailure(t *testing.T) {
	...
	rotated := rotateTURNCandidates(candidates, 1)
	if rotated[0].Transport != turnTransportUDP || rotated[1].Transport != turnTransportTCP {
		t.Fatalf("unexpected fallback order: %#v", rotated)
	}
	if candidates[0].Transport != turnTransportTCP {
		t.Fatal("candidate rotation mutated the original order")
	}
}
