		...
		for _, u := range res.Result {
			...
			if u.CallbackQuery != nil && u.CallbackQuery.Message.Chat.ID == adminID {
				...
				if strings.HasPrefix(data, "viewpass_") {
					...
					dbMutex.Lock()
					for i := 0; i < 64; i++ {
						candidate, generationErr := generatePassword()
					...
					promptMessageID = sendOrEditTelegram(token, adminID, menuMessageID, fmt.Sprintf("🔢 Введите лимит активных ключей.\n\nДопустимо `1..%d`. Лимит равен ёмкости адресного пула.", maxNodeDeviceCapacity), inlineKeyboard(
						[]map[string]interface{}{
							inlineButton("50", "set_limit_50"),
							inlineButton("100", "set_limit_100"),
						},
					...
					if err != nil || validateMaxGeneratedPasswords(limit) != nil {
						promptMessageID = sendSettingsMenu(token, adminID, menuMessageID)
						continue
					}
					dbMutex.Lock()
					...
					generatedLogin, loginErr := generatePassword()
					password, passwordErr := generatePassword()
					if loginErr != nil || passwordErr != nil {
						promptMessageID = sendOrEditTelegram(token, adminID, menuMessageID, "❌ Не удалось безопасно создать учётные данные прокси.", nil)
						continue
					}
					login := "wdtt" + strings.ToLower(generatedLogin[:8])
			...
			if waitingForSetting != "" {
				...
				case "limit":
					...
					if err != nil || validateMaxGeneratedPasswords(limit) != nil {
						waitingForSetting = "limit"
						promptMessageID = sendOrEditTelegram(token, adminID, promptMessageID, fmt.Sprintf("❌ Введите число от `1` до `%d`.", maxNodeDeviceCapacity), inlineKeyboard(
							[]map[string]interface{}{inlineButton("◀️ Назад", "settings")},
						))
						continue
					}
