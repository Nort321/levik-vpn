...
- **Маршрутизация приложений и адресов:** общий режим ЧС/БС применяется к приложениям, точным доменам, URL, IPv4-адресам, подсетям CIDR и диапазонам IPv4. Можно добавлять несколько адресов сразу; списки каждого режима сохраняются отдельно, поддерживают импорт/экспорт и применяются быстрой перезагрузкой WireGuard. Для приложений есть быстрый подбор сервисов, чувствительных к VPN.
...
- **Исключения приложений и адресов:** ЧС и БС хранят независимые наборы приложений, доменов, IPv4-адресов и подсетей. В ЧС выбранное работает напрямую, в БС — направляется в VPN; точные домены преобразуются в их текущие IPv4-маршруты при запуске VPN.
...
Если вы не хотите хранить административные SSH-реквизиты на телефоне,
в будущем сервер можно будет установить из Linux-консоли отдельным
изолированным установщиком — он ещё в разработке. После его выпуска
WDTT Plus будет подключаться по адресу, DTLS-порту и паролю туннеля; SSH
для обычного VPN-подключения не используется.
...
Вкладка **«Исключ.»** содержит отдельные списки приложений и адресов с общим режимом ЧС/БС. В ЧС выбранные приложения и IPv4-направления обходят VPN, в БС через VPN проходят только выбранные элементы; если заполнены обе категории БС, применяется их пересечение. Точные домены преобразуются в IPv4 при запуске WireGuard, а IPv6 и маски доменов пока не поддерживаются. Оба режима сохраняют собственные наборы и поддерживают импорт/экспорт. Отсутствующее после импорта приложение остаётся видимым в списке, чтобы его можно было удалить. WDTT Plus и VK-клиенты всегда обходят VPN, чтобы не ломать TURN-соединение.
